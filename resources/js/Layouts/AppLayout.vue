<template>
    <div class="min-h-screen bg-gray-100">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex">
                        ...
                        <!-- Navigation Links -->
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                            <jet-nav-link :href="route('projects.index')"
                                          :active="$page.currentRouteName.startsWith('projects.')">
                                {{ $t('app.Projects') }}
                            </jet-nav-link>
                        </div>
                    </div>
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ml-6">
                        <!--<a :href="route('locale.change',{'lang': other_lang})">{{ $t(other_lang) }}</a>-->
                        <!--                        <form method="post" @submit.prevent="switchLocale">-->
                        <!--                            <input type="text" name="lang" v-model="this.form.lang">-->
                        <!--                            <button type="submit">{{ $t(other_lang) }}</button>-->
                        <!--                        </form>-->
                        <inertia-link :href="route('locale.change',{'lang': other_lang})" @click="switchLocale"
                                      preserve-state>{{
                                $t(other_lang)
                            }}
                        </inertia-link>
        ...
        <header class="">
            <div class="max-w-7xl mx-auto mb-3 py-4 px-4 sm:px-6 lg:px-8">
...
export default {
    ...
    methods: {
        ...
        switchLocale() {
            ...
            let current_lang = this.$i18n.locale
            //console.log(`current_lang = ${current_lang}`)

            //console.log(`this.other_lang = ${this.other_lang}`)

            // this.form.lang = this.other_lang
            // console.log(`this.form.lang = ${this.form.lang}`)

            this.$i18n.locale = this.other_lang
            //console.log(`this.$i18n.locale after change = ${this.$i18n.locale}`)

            this.other_lang = this.getOtherLang(this.$i18n.locale)
            //console.log(`other_lang after change = ${this.other_lang}`)

            let html_attr = document.getElementsByTagName('html')[0]
            html_attr.setAttribute('lang', this.$i18n.locale)
            html_attr.setAttribute('dir', this.$i18n.locale === 'ar' ? 'rtl' : 'ltr')
            //window.location.href = window.location.origin + new_url
            //this.$inertia.post(window.location.origin + new_url)
            //this.$inertia.post(route('locale.change'), this.form)
            //console.log(`end switchLocale ========`)
        },
        logout() {
    ...
    mounted() {
        this.other_lang = this.getOtherLang(this.$i18n.locale)
        this.form.lang = this.getOtherLang(this.$i18n.locale)
    }
}
</script>
